...
#  @修改时间    : 2023-03-03 09:21:35
#  @上次修改    : 2023/3/3 上午9:21

import json
import uuid
from time import sleep

from src.Common import EncryptTools
...
def getQQServersCallback(url: str, method: int = 0, data={}):
    ...
    """
    return QQHttpServer.getHttp2Json(url, method, data, getHead())
...
def getQQMusicDownloadLinkByTrdServer(mid, sourceType) -> str:
    """
...
def getQQMusicSearch(key: str = "", page: int = 1) -> (dict, dict):
    """搜索音乐
...
def getQQMusicMatchSong(title: dict):
    name = title['name']
    (lst, meta) = getQQMusicSearch(name)
    songs = formatList(lst)
